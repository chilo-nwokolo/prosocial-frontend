...
export default function QuestionsPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    const section = questionSections.find((question) => question.section === decodeURI(params.slug));
    if (!section) {
  ...
  return (
    <Flex flexDir="column" gap="8" mb="5">
      <Flex mt="5">
        <Button variant="link" color="black" onClick={() => router.push(appRouteLinks.intro)}><FaChevronLeft /></Button>
      </Flex>
      <Flex flexDir="column" gap="5">
        <Text fontSize="3xl" fontWeight="semibold">{decodeURI(params.slug)}</Text>
      ...
      <Flex flexDir="column" gap="10">
        {
          section?.questions.data.map((question) => {
            if (question.type === "singleChoice") {
              return <SingleChoiceQuestion key={question.id} id={question.id} title={question.question} options={question?.options} />
            }
            return (
              <RatingScaleQuestion key={question.id} title={question.question} options={question?.options} />
            )
          })
        }
        <Button>Save</Button>
