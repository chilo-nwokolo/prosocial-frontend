...
export default function InterestsPairPage() {
	...
	const selectInterest = (question: Interest) => {
		...
		// return;
		const foundIndex = interestsAnswer.findIndex((interest) => question.id === interest.interest_id);

		if (foundIndex < 0) {
			updateInterestsAnswer([...interestsAnswer, { response: question.title as string, interest_id: question.id as string }]);
		} else if (foundIndex >= 0) {
			const result = [...interestsAnswer];
			result.splice(foundIndex, 1, { response: question.title as string, interest_id: question.id as string });
		...
		<Flex flexDir="column">
			...
			<Flex flexDir="column" px="24" mt="8" gap="10">
				{activeInterest?.interests?.map((question) => (
					<Flex flexDir="column" border="1px solid" borderRadius="lg" height="300px" cursor="pointer" key={question.id} onClick={() => selectInterest(question)}>
						<Box height="250px">
							<Image
